...
class Paylabs_Channel_CIMBVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_cimb_va';
    $this->sub_method_title = 'CIMB Virtual Account';
    $this->sub_icon =  'cimbva.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - CIMB Virtual Account';
    $this->method_description = 'Paylabs CIMB Virtual Account';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
   //  add_action('woocommerce_api_paylabs-cimb-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=cimbva&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
