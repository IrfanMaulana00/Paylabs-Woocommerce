...
class Paylabs_Channel_MuamalatVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_muamalat_va';
    $this->sub_method_title = 'Muamalat Virtual Account';
    $this->sub_icon =  'muamalatva.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - Muamalat Virtual Account';
    $this->method_description = 'Paylabs Muamalat Virtual Account';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
   //  add_action('woocommerce_api_paylabs-muamalat-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=muamalatva&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
