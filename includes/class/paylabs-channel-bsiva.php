...
class Paylabs_Channel_BSIVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_bsi_va';
    $this->sub_method_title = 'BSI Virtual Account';
    $this->sub_icon =  'bsiva.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - BSI VA';
	 $this->method_description = 'Paylabs BSI VA';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
   //  add_action('woocommerce_api_paylabs-bsi-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=bsiva&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
