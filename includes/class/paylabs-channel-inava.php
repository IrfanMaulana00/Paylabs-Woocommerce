...
class Paylabs_Channel_INAVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_ina_va';
    $this->sub_method_title = 'INA Virtual Account';
    $this->sub_icon =  'inava.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - INA VA';
	 $this->method_description = 'Paylabs INA VA';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
    add_action('woocommerce_api_paylabs-ina-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=inava&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
