...
class Paylabs_Channel_PermataVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_permata_va';
    $this->sub_method_title = 'Permata Virtual Account';
    $this->sub_icon =  'permatava.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - Permata VA';
	 $this->method_description = 'Paylabs Permata VA';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
    add_action('woocommerce_api_paylabs-permata-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=permatava&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
