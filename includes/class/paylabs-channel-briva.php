...
class Paylabs_Channel_BRIVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_bri_va';
    $this->sub_method_title = 'BRI Virtual Account';
    $this->sub_icon =  'briva.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - BRI Virtual Account';
    $this->method_description = 'Paylabs BRI Virtual Account';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
   //  add_action('woocommerce_api_paylabs-bri-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=briva&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
