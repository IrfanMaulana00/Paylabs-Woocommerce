...
class Paylabs_Channel_SinarmasVA extends Paylabs_Channel{
    
/**
 * Constructor
 */
 function __construct() {
    $this->id = 'paylabs_sinarmas_va';
    $this->sub_method_title = 'Sinarmas Virtual Account';
    $this->sub_icon =  'sinarmasva.png'; 
    $this->payment_link = '/va/create';
    $this->inquiry_link = '/va/query';
    $this->method_title = 'Paylabs - Sinarmas VA';
	 $this->method_description = 'Paylabs Sinarmas VA';

    //Register webhook for VA result page
    //http://xxxxx.com/wc-api/paylabs-va/
    add_action('woocommerce_api_paylabs-sinarmas-va', array( $this, 'webhook_va_page' ) );
 ...
 function get_after_process_payment_url($response){
   ...
   return get_site_url(null,'/').'?paylabs-mod=sinarmasva&va='.$body['vaCode'].'&id='.$body['merchantTradeNo'].'&rq='.$body['requestId'];
}
