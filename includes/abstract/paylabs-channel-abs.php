     ...
     * Your API interaction could be built with wp_remote_post()
      */
        ...
        if (isset($body->vaCode)) {
            ...
            $file_url = "https://chart.googleapis.com/chart?chs=350x350&cht=qr&choe=UTF-8&chl=" . $body->qrCode;
 ...
 */
    public function webhook_notify_from_paylabs()
    {
        ...
        if ($verifySign === true && $data['errCode'] == '0' && $data['status'] == '02') {
            $order = wc_get_order($data['merchantTradeNo']);
            // $order->payment_complete();
            $order->update_status($this->new_order_status);
            $order->reduce_order_stock();
            $request_id = $data['merchantTradeNo'] . "-" . $data['successTime'];
