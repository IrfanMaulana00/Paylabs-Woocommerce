...
function add_field_order_details($order)
{
	...
	if (!empty($va_number)) {
		...
		if ($fileQr) {
			$data_payment = '<img style="display:inline-block" src="' . $fileQr . '" />';
		} else {
			$data_payment = '<img style="display:inline-block" src="https://chart.googleapis.com/chart?chs=350x350&cht=qr&choe=UTF-8&chl=' . urlencode($qr_code) . '" />';
		}
